"""Evaluate a frozen TaxoLocal-v2 router on the existing locked split.

The repository's original locked split has already been inspected during v1,
so this script marks its output as development comparison rather than a new
blind-test claim.
"""
...
from taxolocal_v2_router import apply_router
...
def main():
    ...
    router_path = os.path.abspath(
        args.router or os.path.join(run_dir, "router_v2", "router.json")
    )
    output_dir = os.path.abspath(
        args.output_dir or os.path.join(run_dir, "test_v2_development")
    )
    metrics_path = os.path.join(output_dir, "metrics.json")
    predictions_path = os.path.join(output_dir, "predictions.jsonl")
    if os.path.exists(metrics_path) or os.path.exists(predictions_path):
        raise FileExistsError("TaxoLocal-v2 test outputs already exist")
    ...
    if any("test" in split for split in router.get(
        "metadata", {}
    ).get("development_splits", [])):
        raise RuntimeError("Router metadata contains a test calibration split")
    if not torch.cuda.is_available():
        raise SystemExit("TaxoLocal-v2 testing requires CUDA")

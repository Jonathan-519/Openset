...
class RegimeEvidenceTests(unittest.TestCase):
    ...
    def test_fit_reference_and_calibration_are_disjoint(self):
        ...
        self.assertEqual(state["method"], "leaf_parent_partial_pooling")
    ...
    def test_conformal_pvalue_ties_are_conservative(self):
        state={"shrinkage":0.,"variants":{"full":{"parents":{"0":{"scores":[1.,2.,3.],"count":3}},"leaves":{"0":{"scores":[1.,2.,3.],"count":3}}}}}
        np.testing.assert_allclose(core.pvalues([0.,1.,2.5,4.],[0,0,0,0],[0,0,0,0],state,"full"),[.25,.5,.75,1.])
    ...
    def test_singleton_branch_inherits_legacy_coverage(self):
        meta,data,rows=fixture(); cfg=settings(); prepared=core.prepare(data,cfg)
        support,fit,_=witness.split_train(rows,cfg,1); y=np.array([r["true_leaf"] for r in rows])
        q=fit[y[fit]==3][:1]; parents=np.array([1])
        e=core.evidence(core.subset(prepared,q),core.subset(prepared,support),y[support],meta["leaf_to_parent"],parents,cfg,candidates=np.array([3]))
        anchor=[dict(rows[int(q[0])],candidate_parent=1,candidate_leaf=3,pred_parent=1,pred_leaf=3,parent=1,leaf=3,prediction_type="known",root_knownness_score=1.,root_gate_margin=1.,child_knownness_score=1.,child_evidence={})]
        out=pipeline.apply({"taxonomy":meta,"settings":cfg},anchor,e,np.array([0.]),"full")
        self.assertEqual(out[0]["prediction_type"],"known")
        anchor[0]["prediction_type"]="fine_unknown"
        out=pipeline.apply({"taxonomy":meta,"settings":cfg},anchor,e,np.array([1.]),"full")
        self.assertEqual(out[0]["prediction_type"],"fine_unknown")

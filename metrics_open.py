...
def _known_group_metrics(records):
    ...
    true_leaf = [int(record["true_leaf"]) for record in records]
    predicted_leaf = [int(record["candidate_leaf"]) for record in records]
    return {
        ...
        "macro_f1": float(f1_score(
            true_leaf, predicted_leaf, average="macro", zero_division=0
        )),
        "balanced_accuracy": float(
            balanced_accuracy_score(true_leaf, predicted_leaf)
        ),
...
def _near_detection_metrics(known_records, intra_records):
    """Metrics for rejecting taxonomically local unseen species."""
    known_records = list(known_records)
    intra_records = list(intra_records)
    labels = np.asarray(
        [1] * len(known_records) + [0] * len(intra_records),
        dtype=np.int64,
    )
    combined = known_records + intra_records
    score_field = (
        "local_knownness_score"
        if combined and all(
            "local_knownness_score" in record for record in combined
        )
        else "local_known_margin"
    )
    scores = np.asarray([
        float(record[score_field]) for record in combined
    ], dtype=np.float64)
    if not len(known_records) or not len(intra_records):
        return {
            "auroc": None,
            "aupr_known": None,
            "aupr_unknown": None,
    ...
    thresholds = np.unique(np.concatenate([
        scores,
        [float(scores.min()) - 1e-9, float(scores.max()) + 1e-9],
    ]))[::-1]
    known_scores = scores[:len(known_records)]
    unknown_scores = scores[len(known_records):]
    ...
    for threshold in thresholds:
        ...
        ccr = float(np.mean(known_correct & (known_scores >= threshold)))
        curve.append((fpr, ccr))
    # For repeated FPR values retain the best achievable CCR.
    by_fpr = {}
    for fpr, ccr in curve:
        by_fpr[fpr] = max(ccr, by_fpr.get(fpr, 0.0))
    xs = np.asarray(sorted(by_fpr), dtype=np.float64)
    ys = np.asarray([by_fpr[x] for x in xs], dtype=np.float64)
    ccr_at_fpr = {}
    for target in (0.01, 0.05, 0.10):
        eligible = ys[xs <= target]
        ccr_at_fpr[str(target)] = (
            0.0 if eligible.size == 0 else float(eligible.max())
        )
    return {
        "auroc": float(roc_auc_score(labels, scores)),
        "aupr_known": float(average_precision_score(labels, scores)),
        "aupr_unknown": float(
            average_precision_score(1 - labels, -scores)
        ),
        "fpr95": _fpr_at_95_tpr(labels, scores),
        "oscr": float(np.sum(
            (xs[1:] - xs[:-1]) * (ys[1:] + ys[:-1]) * 0.5
        )),
        "ccr_at_fpr": ccr_at_fpr,
        "score": score_field,
    }
...
def evaluate_open_set(known_records, intra_records, extra_records):
    ...
    return {
        ...
        "near_open_set": _near_detection_metrics(
            known_records, intra_records
        ),
    }

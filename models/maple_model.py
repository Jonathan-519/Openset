...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def _forward_tokens(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts):
        """Return the final visual sequence before CLS/spatial projection.

        Keeping the patch tokens in the differentiable forward path lets the
        open-set objective learn morphology-sensitive prompts instead of
        extracting frozen post-hoc witnesses.
        """
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts):
        x = self._forward_tokens(x, shared_ctx, compound_deeper_prompts)
    ...
    def forward_with_spatial(
        ...
        """Return projected CLS and patch tokens from the same forward pass."""
        sequence = self._forward_tokens(
            x, shared_ctx, compound_deeper_prompts
        )
        patch_count = int(self.positional_embedding.shape[0]) - 1
        global_features = self.ln_post(sequence[:, 0, :])
        spatial_features = self.ln_post(sequence[:, 1 : 1 + patch_count, :])
        if self.proj is not None:
            global_features = global_features @ self.proj
            spatial_features = spatial_features @ self.proj

...
class CustomCLIP(nn.Module):
    ...
    def encode_image_with_spatial(self, image, normalize=True):
        """Encode CLS and morphology tokens without a second image forward."""
        encoder = self.image_encoder
        if not hasattr(encoder, "forward_with_spatial"):
            raise TypeError(
                "Morphology training requires the ViT MaPLe image encoder"
            )
        shared_ctx, deep_prompts = self.prompt_learner.get_visual_prompt()
        global_features, spatial_features = encoder.forward_with_spatial(
            image.type(self.dtype), shared_ctx, deep_prompts
        )
        if normalize:
...
class MaPLe(nn.Module):
    ...
    def encode_image_with_spatial(self, image, normalize=True):
        return self.model.encode_image_with_spatial(

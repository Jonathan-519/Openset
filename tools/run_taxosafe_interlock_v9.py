"""Frozen v4 caches -> safety-interlocked verifier -> TRAIN conformal calibration -> explicit test.
...
CONFIG = "configs/Zooplankton_Taxonomic_Tree/TaxoSafe_interlock_v9.yml"
...
def make_plan(source_suite, suite, seed, config=CONFIG):
    ...
    paths.update((ROOT / "taxosafe_dual").glob("*.py"))
    paths.update((ROOT / "taxosafe_regime").glob("*.py"))
    paths.update((ROOT / "taxosafe_interlock").glob("*.py"))
    paths.add(Path(__file__))
    paths.add(ROOT / "tools/run_taxosafe_interlock_v9.sh")
    settings = yaml.safe_load(resolve(config).read_text())
    from taxosafe_interlock.core import VARIANTS
    if (settings["variants"] != list(VARIANTS) or settings["classifier"] != "frozen_v4_identity"
            or settings["primary_variant"] != "full" or settings["regularizer"] != 0.01
            or settings["conformal_alpha"] != 0.10 or settings["hierarchical_shrinkage"] != 10.0
            or settings["veto_leaf_multiplier"] != 2.0
            or settings["rescue_parent_multiplier"] != 2.0
            or settings["rescue_leaf_multiplier"] != 4.0):
...
def outputs(plan, stage):
    if stage == "fit":
        return ["models.json", "train_calibration.json", "training_report.json"]
    if stage == "calibrate":
        return ["validation_report.json", "validation_evidence.jsonl"]
    if stage == "test":
...
def test_inputs(plan):
    ...
    if not old_runner.verified(src, "cache_test"):
        raise ValueError("Run the original v4 test cache stage first; v9 never extracts images")
...
def main():
    ...
    p.add_argument("--suite-dir", default="runs/taxosafe_interlock_v9_dev")
    ...
    if args.command == "plan":
        print("Frozen v9 plan:", make_plan(args.source_suite, args.suite_dir, args.seed, args.config))
        ...
        if args.command == "run":
            ...
            verify_inputs(plan)
            from taxosafe_interlock import pipeline

"""Frozen v4 caches -> dual-evidence verifier -> TRAIN conformal calibration -> explicit test.
...
CONFIG = "configs/Zooplankton_Taxonomic_Tree/TaxoSafe_dual_v7.yml"
...
def make_plan(source_suite, suite, seed, config=CONFIG):
    ...
    paths.add(ROOT / "tools/run_taxosafe_dual_v7.sh")
    settings = yaml.safe_load(resolve(config).read_text())
    from taxosafe_dual.core import VARIANTS
    if settings["variants"] != list(VARIANTS) or settings["classifier"] != "frozen_v4_identity" or settings["primary_variant"] != "full" or settings["regularizer"] != 0.01 or settings["conformal_alpha"] != 0.10:
...
def test_inputs(plan):
    ...
    if not old_runner.verified(src, "cache_test"):
        raise ValueError("Run the original v4 test cache stage first; v7 never extracts images")
...
def main():
    ...
    p.add_argument("--suite-dir", default="runs/taxosafe_dual_v7_dev")
    ...
    if args.command == "plan":
        print("Frozen v7 plan:", make_plan(args.source_suite, args.suite_dir, args.seed, args.config))
        ...
        if args.command == "run":
            ...
            verify_inputs(plan)
            from taxosafe_dual import pipeline

...
def build_optimizer(param_groups, optim_cfg):
    ...
    # ``torch.optim.SGD`` requires a numeric dampening value.  The original
    # ProTeCt code used the misspelled key ``sgd_dampning`` and passed None
    # when it was absent; with momentum this crashes on the second step at
    # ``1 - dampening``.  Accept both spellings for backward compatibility.
    sgd_dampening = optim_cfg.get(
        "sgd_dampening",
        optim_cfg.get("sgd_dampning", 0.0),
    )
